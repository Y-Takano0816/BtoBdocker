...
csvdata = pd.read_csv("result/twist_cmd_test", header=None, skiprows=1, usecols=[4, 5, 6, 7, 8, 9]) # 1行目はcsv形式ではないのでスキップする。1-4列目はパラメータではないのでこれもスキップする。行列の番号は0から数えることに注意
csvdata2 = pd.read_csv("result/twist_cmd", header=None, skiprows=1, usecols=[4, 5, 6, 7, 8, 9])

csvdata3 = pd.read_csv("result/twist_raw_test", header=None, skiprows=2, usecols=[4, 5, 6, 7, 8, 9])
csvdata4 = pd.read_csv("result/twist_raw", header=None, skiprows=2, usecols=[4, 5, 6, 7, 8, 9])
...
# '''結果の行列をCSVファイルに書き出し'''
with open("result/twist_cmd_result.csv", "w") as f:
...
# 軸ラベル
ax.set_xlabel("field.twist.linear.x")
ax.set_ylabel("field.twist.linear.y")
...
# 軸ラベル
ax.set_xlabel("field.twist.angular.x")
ax.set_ylabel("field.twist.angular.y")
